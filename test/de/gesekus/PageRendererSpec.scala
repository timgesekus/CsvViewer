  ...
  describe("A PageRenderer") {
    describe("when created with null model") {
      it("should throw an exception") {
        evaluating { new PageRenderer(null) } should produce[IllegalArgumentException]
      }
    }
    describe("when created with empty model") {
      val model = Model()
      it("should throw an exception") {
        evaluating { new PageRenderer(model) } should produce[IllegalArgumentException]
      }
    }
    describe("when created with header line") {
      ...
      val csvView = new PageRenderer(model)

      it("should return a page with only a header and menubar") {
        csvView.page(0,0) should be(
"""Head1    |Head2   |
---------+--------+
Body11   |Body12  |
